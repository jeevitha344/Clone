...
//   if (!movie) return <h1 className="text-white">Loading banner...</h1>;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // fetch full movie details for poster
        const details = await axios.get(
          `?apikey=YOUR_API_KEY&i=${random.imdbID}&plot=full`
        );

        setMovie(details.data);
      } catch (error) {
        console.error("OMDb Banner Error:", error);
      }
    };
